        ...
        <div class="page-main">
            ...
            <span slot="default" class="mock-select" @click="openPop('form', 'checkbox')">
              ...
              <span slot="default" class="mock-select" @click="openPop('education', 'checkbox')">
                <span v-for="(item, index) in education"
                      :key="index">{{index > 0 ? '/' + item.value : item.value}}</span>
              </span>
                        </cell>
              ...
              <span slot="default" class="mock-select" @click="openPop('life', 'checkbox')">
                <span v-for="(item, index) in life" :key="index">{{index > 0 ? '/' + item.value : item.value}}</span>
                        ...
                        <cell title="E 其他" :class="{'not-finish': otherNotFinish}">
                            <input type="text" slot="default" class="mock-ipt" placeholder="请填写" v-model="other"
                                   @change="handleOther">
                        </cell>
                    </div>
            ...
            <span slot="default" class="mock-select" @click="openPop('carsickness', 'radio')">{{carsickness.value}}
                    ...
                    <div>
                        <cell title="A 您对什么食物过敏吗？" :class="{'not-finish': foodallergyNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('foodallergy', 'radio')">{{foodallergy.value}}</span>
                        </cell>
                        <cell title="B 您对什么药物过敏吗？" :class="{'not-finish': drugallergyNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('drugallergy', 'radio')">{{drugallergy.value}}</span>
                        </cell>
                        <cell title="C 您知道自己对什么其他物质过敏吗？" :class="{'not-finish': otherallergyNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('otherallergy', 'radio')">{{otherallergy.value}}</span>
                        </cell>
                    </div>
                </group>
            </div>
            ...
            <div>
                <group>
                    ...
                    <cell title='您吸烟吗？' :class="{'not-finish': isSmokeNotFinish}">
                        <span slot="default" class="mock-select"
                              @click="openPop('isSmoke', 'radio')">{{isSmoke.value}}</span>
                    </cell>
                    <cell title="您饮酒吗？" :class="{'not-finish': isDrinkNotFinish}">
                        <span slot="default" class="mock-select"
                              @click="openPop('isDrink', 'radio')">{{isDrink.value}}</span>
                    </cell>
                    ...
                    <div>
                        <cell title="A 您每日的食盐习惯？" :class="{'not-finish': saltNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('salt', 'radio')">{{salt.value}}</span>
                        </cell>
                        <cell title="B 您日常食用油脂的习惯？" :class="{'not-finish': oilNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('oil', 'radio')">{{oil.value}}</span>
                        </cell>
                        <cell title="C 您经常吃下列哪种食品？(经常指每月4次以上)" :class="{'not-finish': oftenFoodNotFinish}">
                            <span slot="default" class="mock-select"
                                  @click="openPop('oftenFood', 'radio')">{{oftenFood.value}}</span>
                        </cell>
        ...
        <div class="popup" v-show="isPopup" @click="hidePop">
            <div class="popup-main">
                <div class="select-item" v-for="(item, index) in popupOpts" :key="index" @click="stopHide">
                    <label>
                        <input v-if="popupCheckType==='radio'" type="radio" :value="item.key"
                               @change="selectItem(index)" v-model="popChecked">
                        <input v-if="popupCheckType==='checkbox'" type="checkbox" :value="item.key"
                               @change="selectItem(index)" v-model="popChecked">
                        {{item.value}}
                        <input type="text" v-show="item.hasOwnProperty('desc')" v-model="item.desc" class="mock-ipt"
                               placeholder="请描述" @change="selectItem(index)">
                        <span class="checkIcon" v-show="popChecked.indexOf(item.key) >= 0"><x-icon
                                type="ios-checkmark-empty"></x-icon></span>
                    </label>
                </div>
            </div>
        </div>
        ...
        methods: {
            ...
            submit() {
                ...
                let data = {
                    ...
                    activity_scale: this.form.map((item) => {
                        return item.key
                    }).join(','), // 活动规模
                    medical_care: this.medical.map((item) => {
                        return item.key
                    }).join(','), // 医疗
                    education: this.education.map((item) => {
                        return item.key
                    }).join(','), // 教育
                    wealth: this.rich.map((item) => {
                        return item.key
                    }).join(','),//财富
                    life_happy: this.life.map((item) => {
                        return item.key
                    }).join(','), //娱乐
                    other: this.other,//其他

                    three_high: this.threeH.map((item) => {
                        return item.key
                    }).join(','),// 三高
